...
test('a row a fuel with its count, pressed when shown; All and None', async () => {
  ...
  const onHide = vi.fn()
  render(<PlantsPanel plants={plants} hiddenFuels={['coal']} onToggle={onToggle} onHide={onHide} />)
...
test('All is spent when every fuel shows, None when none does', () => {
  const { rerender } = render(<PlantsPanel plants={null} hiddenFuels={[]} onToggle={vi.fn()} onHide={vi.fn()} />)
