...
const record = recordSlot(TEPCO.parse(tepcoCsv), '2026-09-24', 25)

test('without an area, the system price and the range across the areas', () => {
  render(<Readout slot={slot} record={undefined} area={null} onClose={vi.fn()} />)
...
test('a picked area against the system price and its neighbors', async () => {
  const onClose = vi.fn()
  render(<Readout slot={slot} record={undefined} area="tokyo" onClose={onClose} />)
...
test('Okinawa has no price to show', () => {
  render(<Readout slot={slot} record={undefined} area="okinawa" onClose={vi.fn()} />)
  expect(screen.getByText(/Okinawa is not on the exchange/)).toBeInTheDocument()
})

test('nothing before the prices arrive', () => {
  const { container } = render(<Readout slot={undefined} record={undefined} area={null} onClose={vi.fn()} />)
  expect(container).toBeEmptyDOMElement()
})

test('what ran in a picked area, the idle sources left out and the pumping negative', () => {
  render(<Readout slot={slot} record={record} area="tokyo" onClose={vi.fn()} />)
...
test('an area whose company is not wired says so', () => {
  render(<Readout slot={slot} record={undefined} area="kyushu" onClose={vi.fn()} />)
